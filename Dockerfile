# Stage 1: Build the Vite application
FROM node:20-alpine AS build
WORKDIR /app

# Declare build arguments so Selise Blocks can pass environment variables
ARG VITE_BLOCKS_API_URL
ARG VITE_X_BLOCKS_KEY
ARG VITE_PROJECT_SLUG
ARG VITE_MCP_PROXY_URL

ENV VITE_BLOCKS_API_URL=${VITE_BLOCKS_API_URL:-https://api.seliseblocks.com}
ENV VITE_X_BLOCKS_KEY=${VITE_X_BLOCKS_KEY:-f44b9e7d-7c65-4783-a360-14a7df36674e}
ENV VITE_PROJECT_SLUG=${VITE_PROJECT_SLUG:-dryzkn}
ENV VITE_MCP_PROXY_URL=${VITE_MCP_PROXY_URL:-https://unclasp-hypocrite-humility.ngrok-free.dev}

# Copy package files
COPY package*.json ./

# Install dependencies
RUN npm install

# Copy source code
COPY . .

# Build the app
RUN npm run build

# Stage 2: Serve with Nginx
FROM nginx:alpine
# Add custom Nginx configuration to support React Router SPA fallback
COPY nginx.conf /etc/nginx/conf.d/default.conf
# Copy the built assets from the dist folder
COPY --from=build /app/dist /usr/share/nginx/html

EXPOSE 80
CMD ["nginx", "-g", "daemon off;"]
