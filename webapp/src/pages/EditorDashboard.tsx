...
export default function EditorDashboard() {
  ...
  return (
    <div className="flex flex-col min-h-screen relative z-10">
      {/* Top Navigation */}
      <nav className="h-[60px] fixed top-0 w-full z-50 flex items-center justify-between px-6">
      ...
      <div className="flex flex-1 pt-[80px] max-w-[1400px] w-full mx-auto px-4 md:px-6 gap-6 pb-8 h-screen">
        ...
        {/* Main Content */}
        <GlowCard customSize glowColor="blue" className="flex-1 h-full !p-0">
...
function ProfileEditor({ profile, setProfile }: { profile: any, setProfile: any }) {
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in-up">
      <header>
        <span className="text-xs font-semibold tracking-wider text-white/40 uppercase mb-2 block">Identity</span>
        <h1 className="text-3xl font-semibold text-white tracking-tight">Profile details</h1>
      </header>

      <div className="space-y-6 max-w-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">Display Name</label>
            <input 
              type="text" 
              name="displayName"
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all placeholder:text-white/20" 
          ...
          <div className="space-y-2">
            <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">Username</label>
            <input 
              type="text" 
              name="username"
              className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all placeholder:text-white/20" 
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">Headline</label>
          <input 
            type="text" 
            name="headline"
            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all placeholder:text-white/20" 
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">About Me</label>
          <textarea 
            name="bio"
            className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all placeholder:text-white/20 min-h-[160px] resize-y" 
            placeholder="Write a concise public bio. Line breaks are preserved."
            value={profile.bio}
            onChange={handleChange}
          />
        </div>
...
function MediaEditor({ profile, setProfile }: { profile: any, setProfile: any }) {
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in-up">
      <header>
        <span className="text-xs font-semibold tracking-wider text-white/40 uppercase mb-2 block">Assets</span>
        <h1 className="text-3xl font-semibold text-white tracking-tight">Media</h1>
      </header>

      <div className="space-y-8 max-w-2xl">
        <div className="space-y-4">
          <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">Profile Picture</label>
          <div className="flex items-center space-x-6">
            <div className="w-24 h-24 rounded-full border border-white/10 bg-white/5 flex items-center justify-center overflow-hidden shrink-0">
              {profile.avatar ? (
                <img src={profile.avatar} alt="Avatar" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="flex-1">
              <label className="relative group rounded-xl border border-dashed border-white/20 bg-white/5 hover:bg-white/10 transition-all p-6 text-center cursor-pointer block">
                <input 
                  type="file" 
                  className="hidden" 
                  accept="image/*"
                ...
                />
                <p className="text-sm font-medium text-white/80">Click to browse or drag image</p>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <label className="text-xs font-medium text-white/50 uppercase tracking-wider block">Header Image</label>
          <label className="relative rounded-xl border border-dashed border-white/20 bg-white/5 overflow-hidden h-32 group block cursor-pointer">
            <input 
              type="file" 
              className="hidden" 
              accept="image/*"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center hover:bg-white/5 transition-all">
              <ImageIcon className="w-6 h-6 mx-auto mb-3 text-white/40" />
              <p className="text-sm font-medium text-white/80">Upload a cover photo</p>
            </div>
          </label>
        </div>
      </div>
...
function LinksEditor({ profile, setProfile }: { profile: any, setProfile: any }) {
  ...
  return (
    <div className="p-8 space-y-8 animate-fade-in-up">
      <header>
        <span className="text-xs font-semibold tracking-wider text-white/40 uppercase mb-2 block">Connect</span>
        <h1 className="text-3xl font-semibold text-white tracking-tight">Social Links</h1>
      </header>

      <div className="space-y-6 max-w-2xl">
        <div className="space-y-4">
          {(profile.links || []).map((link: any) => (
            <div key={link.id} className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-4 animate-fade-in">
              <input 
                type="text" 
